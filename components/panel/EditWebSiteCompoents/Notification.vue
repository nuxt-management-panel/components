...
export default {
  ...
  data() {
    return {
      ...
      url: '/website/notification_component',
      modal: null,
      modalTitle: null,
      editor: null,
      modalType: 'add',
  ...
  mounted() {
    ClassicEditor
        .create(document.querySelector('#notificationEditor'), {
          language: {
            content: 'ar'
          },
        ...
        .then(newEditor => {
          window.editorNotification = newEditor;
        })
  ...
  methods: {
    ...
    addModal() {
      this.modalTitle = "افزودن اطلاعیه جدید"
      this.modalType = 'add'
      Object.keys(this.body).forEach(key => this.body[key] = null);
      this.modal.show()
    },
    editModal(item) {
      this.modalTitle = "ویرایش اطلاعیه"
      this.modalType = 'edit'
      this.body = {...item}
      this.body.date = this.gregorian_to_jalali(this.body.date)
      window.editorNotification.setData(this.body.text)
      this.modal.show()
    },
    submit() {
    ...
    async add() {
      ...
      formData.append('text', window.editorNotification.getData())
      if (this.body.date)
